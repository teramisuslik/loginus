@echo off
echo ========================================
echo Развертывание исправлений GitHub и Email
echo ========================================
echo.
...
echo.
echo 7. Жду 5 секунд для запуска контейнера...
timeout /t 5 /nobreak >nul

echo.
echo 8. Проверяю логи контейнера (последние 30 строк)...
ssh -i C:\Users\teramisuslik\.ssh\id_ed25519 root@45.144.176.42 "docker logs loginus-backend --tail 30"
echo.

echo ========================================
echo Готово! Исправления развернуты.
echo ========================================
echo.
echo Исправлено:
echo - Регистрация через GitHub теперь работает даже если email уже используется
echo - Привязка email обрабатывает конфликты корректно
echo - После привязки GitHub не требуется повторный вход с nFA
echo.
pause
